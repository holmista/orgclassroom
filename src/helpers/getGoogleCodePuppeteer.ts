...
async function getGoogleCode(): Promise<string> {
  ...
  const passwordInputSelector = "#password > div.aCsJod.oJeWuf > div > div.Xb9hP > input";
  await page.waitForSelector(emailInputSelector);
  await page.click(emailInputSelector);
  await page.$eval(emailInputSelector, (el) => (el.value = "forTestingPurposes317@gmail.com"));
  ...
  const elem = await page.waitForSelector("#passwordNext > div > button > div.VfPpkd-Jh9lGc");
  elem &&
    page.evaluate((btn) => {
      btn.click();
    }, elem);
