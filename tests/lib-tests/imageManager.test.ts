...
test("return error when writing image with unsupported type", async () => {
  ...
  await expect(imageManager.writeImage(1, 1, 1, image)).rejects.toThrowError("image type not supported");
});
...
test("return error when writing image in directory which does not exist", async () => {
  ...
  await expect(imageManager.writeImage(1, 1, 1, image)).rejects.toThrowError("path not found");
});
...
test("write image in directory which exists", async () => {
  ...
  await expect(imageManager.writeImage(1, 1, 1, image)).resolves.toBeUndefined();
...
test("return error when writing images and one of them has unsupported type", async () => {
  ...
  await expect(imageManager.writeImages(1, 1, 1, images)).rejects.toThrowError("image type not supported");
});
...
test("return error when writing images and directory does not exist", async () => {
  ...
  await expect(imageManager.writeImages(1, 1, 1, images)).rejects.toThrowError("path not found");
});
...
test("write images in directory which exists", async () => {
  ...
  await fs.mkdir("storage/1/1/1", { recursive: true });
  await expect(imageManager.writeImages(1, 1, 1, images)).resolves.toBeUndefined();
  const files = await fs.readdir("storage/1/1/1");
  expect(files).toHaveLength(2);
});

test("return error when reading image which does not exist", async () => {
  await expect(imageManager.readImage(1, 1, 1, "test.png")).rejects.toThrowError("path not found");
});

test("read image which exists", async () => {
  await fs.mkdir("storage/1/1/1", { recursive: true });
  await fs.writeFile("storage/1/1/1/test.png", "test");
  await expect(imageManager.readImage(1, 1, 1, "test.png")).resolves.toBeInstanceOf(Buffer);
});

test("return error when reading images which do not exist", async () => {
  await expect(imageManager.readImages(1, 1, 1)).rejects.toThrowError("path not found");
});
...
test("return error when deleting image which does not exist", async () => {
  await expect(imageManager.deleteImage(1, 1, 1, "test.png")).rejects.toThrowError("path not found");
});

test("return error when deleting file with unsupported image type", async () => {
  await expect(imageManager.deleteImage(1, 1, 1, "test.gif")).rejects.toThrowError("image type not supported");
});

test("delete image which exists", async () => {
  await fs.mkdir("storage/1/1/1", { recursive: true });
  await fs.writeFile("storage/1/1/1/test.png", "test");
  await expect(imageManager.deleteImage(1, 1, 1, "test.png")).resolves.toBeUndefined();
  const files = await fs.readdir("storage/1/1/1");
  expect(files).toHaveLength(0);
});

test("return error when deleting images which do not exist", async () => {
  await expect(imageManager.deleteImages(1, 1, 1)).rejects.toThrowError("path not found");
});
