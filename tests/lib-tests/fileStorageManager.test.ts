...
test("return error if path not found when reading a file", async () => {
  await expect(
    fileStorageManager.readFile(1, 1, 1, "test.txt")
  ).rejects.toThrow("path not found");
});
...
test("return error if path not found when reading files", async () => {
  await expect(fileStorageManager.readFiles(1, 1, 1)).rejects.toThrow(
    "path not found"
  );
});
...
test("read files if path found when reading files", async () => {
  ...
  await expect(fileStorageManager.readFiles(1, 1, 1)).resolves.toEqual([
    Buffer.from("test"),
    Buffer.from("test2"),
  ]);
});
...
test("return error if path not found when deleting a file", async () => {
  await expect(
    fileStorageManager.deleteFile(1, 1, 1, "test.txt")
  ).rejects.toThrow("path not found");
});
...
test("delete file if path found when deleting a file", async () => {
  ...
  await expect(fs.access("storage/1/1/1/test.txt")).rejects.toThrow(
...
test("throw error if folder already exists when creating a user folder", async () => {
  ...
  await expect(fileStorageManager.createUserFolder(1)).rejects.toThrow(
    "folder already exists"
  );
});

test("throw error when deleting a user folder that does not exist", async () => {
  await expect(fileStorageManager.deleteUserFolder(1)).rejects.toThrow(
    "path not found"
  );
});
...
test("throw error if folder already exists when creating a subject folder", async () => {
  await fs.mkdir("storage/1/1", { recursive: true });
  await expect(fileStorageManager.createSubjectFolder(1, 1)).rejects.toThrow(
    "folder already exists"
  );
});
...
test("throw error when deleting a subject folder that does not exist", async () => {
  await expect(fileStorageManager.deleteSubjectFolder(1, 1)).rejects.toThrow(
    "path not found"
  );
});
...
test("create folder when creating a subject folder which does not exist and user exists", async () => {
  await fs.mkdir("storage/1", { recursive: true });
  await expect(
    fileStorageManager.createSubjectFolder(1, 1)
  ).resolves.toBeUndefined();
});
...
test("delete folder when deleting a subject folder which exists and user exists", async () => {
  await fs.mkdir("storage/1/1", { recursive: true });
  await expect(
    fileStorageManager.deleteSubjectFolder(1, 1)
  ).resolves.toBeUndefined();
});
